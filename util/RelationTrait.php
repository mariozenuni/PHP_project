...
trait RelationTrait
{
	...
	public function loadAll($POST, $skippedRelations = [])
	{
		if ($this->load($POST)) {
			...
			foreach ($POST[$shortName] as $model => $attr) {

				if (is_array($attr)) {

					$models[count($models)] = new $this($attr);
					if ($model == $shortName) {
	 ...
	 * */
	public static function findOrCreate($condition,array $attributes = [])
	{
	...
	public function saveOrfail()
	{
		...
		if(!$this->save()){
			...
			$errors= ArrayUtil::array_flatten($errors);
			$errors = implode(',',$errors);
			throw new Exception("
				Si sono verificati i seguenti errori : $errors
			");
		}
	...
	/**
	 * @param array $condition the condition to search the model
	 * @param array $attributes the attributes to set
	 *
	 * it doesnt' directly save the model!
	 * */
	public static function updateOrCreate(array $condition,array $attributes)
	{

		$model = self::findOrCreate($condition,$attributes);
